...
def validate_posts_meta(infos):
  expected_keys = ['title', 'date', 'tags']
  ...
  for info in infos:
    ...
    # capture any missing important meta data
    for expected_key in expected_keys:
      if expected_key not in meta_keys:
        raise Exception(
          f'meta data "{expected_key}" is missing from "{post}"')

    tags = meta['tags']
    seen = set()

    # capture any duplications in post tags
    for tag in tags:
      if tag in seen:
        raise Exception(
          f'the tag "{tag}" is duplicated in "{post}"')
      seen.add(tag)
